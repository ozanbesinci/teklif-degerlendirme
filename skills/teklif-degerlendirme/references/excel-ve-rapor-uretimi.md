...
`references/teslim-ve-dogrulama.md` Adım 11'deki listeye ek mekanik kontroller:
...
olmalı. Doğrulanmamış PDF teslim edilmiş sayılmaz (`references/teslim-ve-dogrulama.md` Adım 11/18).
...
**v3.0.1:** Aşağıdaki tablo eski ortamların tipik araç imkânlarını tarif eder;
güncel erişim/abonelik/kurulum garantisi değildir. Her çalışmada dosya, kod,
internet ve alt ajan yetenekleri fiilen kontrol edilir. Tam model/efor profili
`references/ajan-mimarisi.md` ile `references/ajan-calistirma.md` kurallarına tabidir.
Model veya bağımsız denetçi seçilemeyen ortamda tam v3 doğrulaması iddia edilmez.
